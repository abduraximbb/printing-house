...
import { FindByTime } from "./dto/findbytime-client.dto";
...
export class ClientsService {
  ...
  async findByTime(id:number,findByTimeDto:FindByTime){
    const client = await this.clientModel.findOne({where:{id}})
    if(!client){
      throw new BadRequestException("This client not found")
    }
    const first_time = new Date(findByTimeDto.start_time)
    const second_time  = new Date(findByTimeDto.end_time)
    const orders = await this.orderModel.findAll({where:{clientId:id}})
    if(!orders){
      throw new BadRequestException("Bu clientning orderi yo'q")
    }
    ...
    if(orders_list.length==0){
      return new BadRequestException("Bu vaqt oralig'ida order qilmagan")
    }
